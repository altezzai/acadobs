...
import '../../../../base/utils/date_formatter.dart';

class UpdateStudentPage extends StatefulWidget {
  final int studentId;

  const UpdateStudentPage({super.key, required this.studentId});
  @override
...
class _UpdateStudentPageState extends State<UpdateStudentPage> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      final student = studentController.individualStudent;
      if (student != null) {
        ...
        _dateOfJoiningController.text =
            DateFormatter.formatDateString(student.createdAt.toString());
        _dateOfBirthController.text =
            DateFormatter.formatDateString(student.dateOfBirth.toString());
        _addressController.text = student.residentialAddress ?? '';
        _aadhaarNumberController.text = student.aadhaarNumber ?? '';
        _emailController.text = student.email ?? '';
        _fatherFullNameController.text = student.fatherFullName ?? '';
        _motherFullNameController.text = student.motherFullName ?? '';
        _guardianNameController.text = student.guardianFullName ?? '';
        _fatherContactNumberController.text = student.fatherContactNumber ?? '';
        _motherContactNumberController.text = student.motherContactNumber ?? '';
        parentEmailController.text = student.parentEmail ?? '';

        // Populate dropdowns
        dropdownProvider.setSelectedItem('gender', student.gender ?? '');
        dropdownProvider.setSelectedItem('class', student.studentClass ?? '');
        dropdownProvider.setSelectedItem('division', student.section ?? '');
      }
    });
  }
            ...
            child: Column(
              ...
              children: [
                ...
                Consumer<StudentController>(builder: (context, value, child) {
                  return _sectionTitle(value.individualStudent?.fullName ?? "");
                }),
                SizedBox(height: 10),
  ...
  void _submitForm(BuildContext context) {
    ...
    if (_formKey.currentState?.validate() ?? false) {
      try {
        ...
        // Call the controller method to update the student
